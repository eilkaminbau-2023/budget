...
export default function ZahlungBearbeiten() {
  ...
  const [mietverhaeltnis, setMietverhaeltnis] = useState<any>(null);
  ...
  useEffect(() => {
    if (id) {
      fetch(`/api/zahlungen/${id}`)
        .then(res => res.json())
        .then(data => {
          console.log("Geladene Daten:", data); // Debug için
          
          // 🔥 STATUS DÖNÜŞÜMÜ: API'den gelen status'ü select option'a uygun hale getir
          let status = data.status;
          // API'den gelen değerler: BEZAHLT, AUSSTEHEND, VERSPAETET
          // Select option'lar: BEZAHLT, AUSSTEHEND, VERSPAETET olarak düzeltildi
          ...
          setFormData({
            ...
            betrifftMonat: data.betrifftMonat || '',
            mietverhaeltnisId: data.mietverhaeltnisId || ''
          });
          
          // Mietverhältnis bilgilerini de sakla (göstermek için)
          if (data.mietverhaeltnis) {
            setMietverhaeltnis(data.mietverhaeltnis);
          }
        })
        .catch(err => {
      ...
      >
        <div className="flex items-center justify-between mb-8 border-b border-gray-100 pb-4">
          <div>
            ...
            {/* 🔥 Mietverhältnis bilgisi göster */}
            {mietverhaeltnis && (
              <p className="text-sm text-gray-500 mt-1">
                {mietverhaeltnis.mietobjekt?.adresse} - {mietverhaeltnis.mieter?.name}
              </p>
            )}
          </div>
        ...
        {/* 🔥 DÜZELTİLDİ: STATUS SELECTION - API ile uyumlu */}
        <div className="space-y-2">
          <label className="block text-sm font-bold text-gray-700 uppercase tracking-wide">
            Zahlungsstatus
          </label>
          ...
          >
            ...
            <option value="VERSPAETET">⚠️ Verspätet</option>
          </select>
          <p className="text-xs text-gray-500 mt-1">
            Aktueller Status: <span className="font-bold">{formData.status}</span>
          </p>
        </div>
